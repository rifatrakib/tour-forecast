...
target_metadata = [District.metadata]
...
async def execute_migrations(connection_config: Dict[str, Any]) -> None:
...
async def run_async_migrations() -> None:
    ...
    try:
        ...
        await execute_migrations(connection_config)
    except InvalidAuthorizationSpecificationError:
        print("rerun with localhost...")
        connection_config = config.get_section(config.config_ini_section, {})
        connection_config["sqlalchemy.url"] = settings.RDS_URI.replace(settings.POSTGRES_HOST, "localhost")
        await execute_migrations(connection_config)
